...
int main()
{   
    writef("Assignment Problem Solver: (- Dhiraj Bag, Roll: 001911001033) \n");
    writef("\nEach row represents a candidate, ie. num(candidates)=num(rows)\n");
    writef("Each column represents a job, ie. num(jobs)=num(columns)\n");

    int num_rows, num_cols;
    writef("\nEnter num(Rows) <space> num(Columns): ");
    readf("%d %d", &num_rows, &num_cols);
    ...
    if (isRestricted)
    {
        ...
        writef("Enter the number of restrictions: ");
        readf("%d", &numRestrictions);

        writef("Enter the %d restricted row_number <space> column_number in %d lines(numbering starts from 1) : \n", numRestrictions, numRestrictions);
    ...
    while (1)
    {
        ...
        */
        ...
        { //Should never happen
            writef("Error: Unexpected behaviour : numLines > num_rows");
            return 0;
        }

        if(numLines < num_cols)
        { /* numLines < num_rows : Further optimization needed */
            writef("Optimal condition not reached. Doing necessary changes to cost matrix ...\n");
